...
public class GSellGump_InventoryItem : GRegion
{
	...
	{
		if (this.m_Info.ToSell < this.m_Info.Amount)
		{
			SellInfo info = this.m_Info;
			int toSell = info.ToSell + 1;
			info.ToSell = toSell;
			if (this.m_Info.OfferedGump == null)
			{
				this.m_Info.OfferedGump = new GSellGump_OfferedItem(this.m_Owner, this.m_Info);
				this.m_Owner.OfferMenu.Children.Add(this.m_Info.OfferedGump);
			}
			this.m_Info.OfferedGump.Amount = this.m_Info.ToSell;
			toSell = this.Available - 1;
			this.Available = toSell;
