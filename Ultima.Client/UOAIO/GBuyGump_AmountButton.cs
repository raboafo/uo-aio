...
public class GBuyGump_AmountButton : GImage
{
	...
	protected internal override void OnMouseEnter(int x, int y, MouseButtons mb)
	{
		if ((mb & MouseButtons.Left) != MouseButtons.None)
		{
			...
			this.m_Info.ToBuy += Math.Sign(this.m_Offset);
			this.AmountChanged();
		}
	}
	...
	{
		if (this.m_Info.ToBuy <= 0)
		{
			this.m_Info.ToBuy = 0;
			this.m_Info.OfferedGump = null;
			Gumps.Destroy(base.m_Parent);
		}
		else if (this.m_Info.ToBuy > this.m_Info.Amount)
		{
			this.m_Info.ToBuy = this.m_Info.Amount;
		}
		this.m_Info.InventoryGump.Available = this.m_Info.Amount - this.m_Info.ToBuy;
	...
	protected internal override void Draw(int x, int y)
	{
		...
		{
			this.m_Info.dToBuy += (Engine.dTicks - this.m_Last) / 1000.0 * this.m_Offset;
