...
public class GHairHuePickerAll : Gump
{
	private const int xShades = 6;
	...
	public int Hue => 1102 + (this.m_yShade * 6 + this.m_xShade) * 8 + this.m_zShade;

	public override int Width => this.xSize * 6;
	...
	public GHairHuePickerAll(int X, int Y, int Width, int Height)
		: base(X, Y)
	{
		do
		{
		...
		this.ySize--;
		this.m_ColorTable = new int[6, 1, 8];
		for (int i = 0; i < 6; i++)
		{
			for (int j = 0; j < 1; j++)
			{
				for (int k = 0; k < 8; k++)
				{
					ushort num = Hues.GetData(1101 + (j * 6 + i) * 8 + k).colors[48];
