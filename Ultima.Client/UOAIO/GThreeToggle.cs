...
public class GThreeToggle : Gump
{
	...
	protected VertexCachePool VCPool => GThreeToggle.m_vPool;
	...
	public override int Width => this.m_Sizes[this.m_State].Width;

	public override int Height => this.m_Sizes[this.m_State].Height;
	...
	public GThreeToggle(int state0, int state1, int state2, int initialState, IHue hue, int x, int y)
		: base(x, y)
	{
		this.m_Hue = hue;
		this.m_Images = new Texture[3];
		this.m_Images[0] = this.m_Hue.GetGump(state0);
		this.m_Images[1] = this.m_Hue.GetGump(state1);
		this.m_Images[2] = this.m_Hue.GetGump(state2);
		this.m_Draw = new bool[3];
		this.m_Draw[0] = this.m_Images[0] != null && !this.m_Images[0].IsEmpty();
		this.m_Draw[1] = this.m_Images[1] != null && !this.m_Images[1].IsEmpty();
		this.m_Draw[2] = this.m_Images[2] != null && !this.m_Images[2].IsEmpty();
	...
	static GThreeToggle()
	{
		GThreeToggle.m_vPool = new VertexCachePool();
	}
}
